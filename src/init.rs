...
use nu_engine::{filesystem::filesystem_shell::FilesystemShellMode, EvaluationContext};
...
pub fn init_from_args(matches: &ArgMatches) -> Result<EvaluationContext, ShellError> {
    ...
    let ctx = create_default_context_from_matches(matches)?;
    ...
    if let Some(cfg) = matches.value_of("config-file") {
        load_cfg_as_global_cfg(&ctx, PathBuf::from(cfg));
    } else {
        load_global_cfg(&ctx);
    }
...
fn create_default_context_from_matches(
    matches: &ArgMatches,
) -> Result<EvaluationContext, ShellError> {
    ...
    let context = create_default_context(mode, true)?;
