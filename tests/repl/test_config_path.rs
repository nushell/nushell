...
#[cfg(not(windows))]
#[deps(NU)]
fn test_xdg_config_symlink(playground: Playground) -> Result {
    ...
    let result: CompleteResult = test()
        .env(
            "XDG_CONFIG_HOME",
            playground.path().join(config_link).display().to_string(),
        )
        .inherit_env_if_set("HOME")
        .run("nu -i -c 'echo $nu.is-interactive' | complete")?;
...
#[test]
fn config_home_cli_affects_default_config_dir(playground: Playground) -> Result {
    let alt_home = playground.path().join("alt-config-home");
...
#[test]
fn config_home_cli_affects_history_path(playground: Playground) -> Result {
    let alt_home = playground.path().join("alt-hist-home");
    std::fs::create_dir_all(&alt_home)?;
...
#[test]
#[deps(NU)]
fn dangling_config_nu_uses_defaults_with_warning(playground: Playground) -> Result {
    let xdg = playground.path().join("xdg");
...
#[test]
#[deps(NU)]
fn dangling_env_nu_uses_defaults_with_warning(playground: Playground) -> Result {
    let xdg = playground.path().join("xdg");
    let nu_dir = xdg.join("nushell");
    fs::create_dir_all(&nu_dir)?;
...
#[test]
#[deps(NU)]
fn dangling_config_cli_override_still_errors(playground: Playground) -> Result {
    playground.symlink("/nonexistent/custom-config.nu", "broken-config.nu")?;
    let broken = playground.path().join("broken-config.nu");
