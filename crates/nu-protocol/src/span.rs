...
pub trait IntoSpanned: Sized {
    ...
    ///
    /// ```
    /// # use nu_protocol::{FutureSpanId, IntoSpanned};
    ...
    /// ```
    fn into_spanned(self, span: FutureSpanId) -> Spanned<Self>;
}
...
impl FutureSpanId {
    ...
    /// when used in errors.
    pub const fn test_data() -> FutureSpanId {
        Self::unknown()
    }
