...
impl Value {
                        ...
                        for val in vals.iter_mut() {
                            match val {
                                Value::Record { val, .. } => {
                                    ...
                                    if !found {
                                        if cell_path.len() == 1 {
                                            val.push(col_name.clone(), new_val);
                                            break;
                                        } else {
                                            let mut new_col =
                        ...
                        for val in vals.iter_mut() {
                            match val {
                                Value::Record { val, span: v_span } => {
                                    ...
                                    val.push(col_name.clone(), new_val.clone());
                                }
                                // SIGH...
    ...
    pub fn record(record: Record, span: Span) -> Value {
        Value::Record {
            val: Box::new(record),
            span,
        }
    ...
    pub fn record_from_iter(iter: impl IntoIterator<Item = (String, Value)>, span: Span) -> Value {
        Value::record(Record::from_iter(iter), span)
    }

    pub fn record_from_parts(cols: Vec<String>, vals: Vec<Value>, span: Span) -> Value {
    ...
    /// Note: Only use this for test data, *not* live data, as it will point into unknown source
    /// when used in errors.
    pub fn test_record_from_iter(iter: impl IntoIterator<Item = (String, Value)>) -> Value {
        Self::record_from_iter(iter, Span::test_data())
    }

    /// Note: Only use this for test data, *not* live data, as it will point into unknown source
    /// when used in errors.
    pub fn test_record_from_parts(cols: Vec<impl Into<String>>, vals: Vec<Value>) -> Value {
        Self::record_from_parts(
        ...
        match (self, other) {
            ...
            (Value::Record { val: lhs, .. }, rhs) => match rhs {
                ...
                Value::Record { val: rhs, .. } => {
                    ...
                    let (lhs_cols, lhs_vals) = reorder_record_inner(lhs);
                    let (rhs_cols, rhs_vals) = reorder_record_inner(rhs);
...
impl Value {
    ...
    pub fn r#in(&self, op: Span, rhs: &Value, span: Span) -> Result<Value, ShellError> {
        match (self, rhs) {
            ...
            (Value::String { val: lhs, .. }, Value::Record { val: record, .. }) => {
                Ok(Value::Bool {
                    val: record.cols.contains(lhs),
                    span,
                })
            }
