...
use crate::{
    ast::ImportPattern, engine::EngineState, BlockId, FutureSpanId, OutDest, Signature, Spanned,
    VarId,
};
...
pub enum Expr {
    ...
    VarDecl(VarId),
    Call(Box<Call>),
    ExternalCall(Box<Expression>, Box<Spanned<Vec<ExternalArgument>>>), // head, args
    Operator(Operator),
