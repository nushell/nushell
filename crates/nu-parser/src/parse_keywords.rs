...
pub fn parse_for(working_set: &mut StateWorkingSet, spans: &[Span]) -> Expression {
    ...
    let (call, call_span) = match working_set.find_decl(b"for") {
        ...
        Some(decl_id) => {
            ...
            if starting_error_count != working_set.parse_errors.len() || call.has_named("help") {
                return Expression {
                    expr: Expr::Call(call),
                    span: call_span,
                    ty: output,
...
pub fn parse_def(
    ...
    let has_env = call.has_named("env");
    let has_wrapped = call.has_named("wrapped");
...
pub fn parse_alias(
    ...
    if let Some(decl_id) = working_set.find_decl(b"alias") {
        ...
        let has_help_flag = alias_call.has_named("help");
...
pub fn parse_overlay_use(working_set: &mut StateWorkingSet, call: Box<Call>) -> Pipeline {
    ...
    let has_prefix = call.has_named("prefix");
    let do_reload = call.has_named("reload");
...
pub fn parse_overlay_hide(working_set: &mut StateWorkingSet, call: Box<Call>) -> Pipeline {
    ...
    let keep_custom = call.has_named("keep-custom");
