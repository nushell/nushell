    ...
    match &expr.expr {
        ...
        Expr::Table(headers, vals) => {
            ...
            for val in vals {
                ...
                output_rows.push(Value::record_from_parts(
...
pub fn eval_variable(
    ...
    match var_id {
        ...
        ENV_VARIABLE_ID => {
            ...
            let mut pairs = Vec::from_iter(env_vars);
            pairs.sort_by(|(col1, _), (col2, _)| col1.cmp(col2));
            Ok(Value::record_from_iter(pairs, span))
        }
...
fn collect_profiling_metadata(
    ...
    let element_metadata = eval_result
        .as_ref()
        .ok()
        .and_then(|(pipe, _)| pipe.metadata().map(|s| *s));
