...
use nu_protocol::{
    ...
    eval_base::Eval,
    Config, DeclId, IntoPipelineData, IntoSpanned, PipelineData, RawStream, ShellError, Signature,
    Span, Spanned, Type, Value, VarId, ENV_VARIABLE_ID,
};
use std::thread::{self, JoinHandle};
use std::{borrow::Cow, collections::HashMap};

pub fn bind_args_to_stack(
    stack: &mut Stack,
    signature: &Signature,
    args: Vec<Value>,
    head_span: Span,
...
pub fn eval_call(
    ...
    if !decl.is_known_external() && call.named_iter().any(|(flag, _, _)| flag.item == "help") {
        ...
        // let mut args = vec![];
        // for idx in 0..signature.required_positional.len() + signature.optional_positional.len() {
        //     if let Some(arg) = call.positional_nth(idx) {
        //         args.push(eval_expression(engine_state, caller_stack, arg)?)
        //     }
        // }
        // if let Some(rest_positional) = decl.signature().rest_positional {
        //     args.append(&mut call.rest_iter_flattened(
        //         decl.signature().required_positional.len()
        //             + decl.signature().optional_positional.len(),
        //         |expr| eval_expression(engine_state, caller_stack, expr),
        //     )?)
        // }
        // bind_args_to_stack(&mut callee_stack, &signature, args, call.head)?;
