...
use nu_protocol::{
    ...
    engine::{Closure, EngineState, Stack, DB_VARIABLE_ID},
                    ...
                    if let Ok(conn) = db.open_connection() {
                        ...
                        .map_err(|err| {
                            ShellError::GenericError(
                                "Failed to open SQLite connection in memory from eval".into(),
                                err.to_string(),
                                Some(Span::test_data()),
                                None,
                                Vec::new(),
                        ...
                        let mut stmt = conn
                            ...
                            .map_err(|err| {
                            ShellError::GenericError(
                                "Failed to prepare initial $db insert statement".into(),
                                err.to_string(),
                                Some(Span::test_data()),
                                None,
                                Vec::new(),
                        ...
                        let _rows_inserted = stmt.execute([]).map_err(|err| {
                            ShellError::GenericError(
                                "Failed to insert initial record into $db".into(),
                                err.to_string(),
                                Some(Span::test_data()),
                                None,
                                Vec::new(),
