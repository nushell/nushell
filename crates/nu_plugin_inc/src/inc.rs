...
impl Inc {
        ...
        match &self.action {
            Some(Action::SemVerAction(act_on)) => {
                let mut ver = match semver::Version::parse(input) {
                    ...
                    Err(_) => return Value::String(input.to_string()),
                };
            ...
            Some(Action::Default) | None => match input.parse::<u64>() {
                Ok(v) => Value::String((v + 1).to_string()),
    ...
    pub fn inc_value(&self, head: Span, value: &Value) -> Result<Value, LabeledError> {
        match value {
            Value::Int(val) => Ok(Value::Int(val + 1)),
            Value::String(val) => Ok(self.apply(val, head)),
            x => {
                let msg = x.as_string(head).map_err(|e| LabeledError {
...
mod tests {
    mod semver {
        ...
        #[test]
        fn major() {
            let expected = Value::String("1.0.0".to_string());
            let mut inc = Inc::new();
        ...
        #[test]
        fn minor() {
            let expected = Value::String("0.2.0".to_string());
            let mut inc = Inc::new();
        ...
        #[test]
        fn patch() {
            let expected = Value::String("0.1.4".to_string());
            let mut inc = Inc::new();
