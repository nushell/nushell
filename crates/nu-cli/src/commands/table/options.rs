...
use std::fmt::Debug;
use nu_source::Tag;
use nu_protocol::{Value, UntaggedValue};
...
pub fn header_alignment_from_value(align_value: Option<&Value>) -> nu_table::Alignment {
    match align_value {
        Some(v) => match v.as_string().unwrap_or("none".to_string()).as_ref() {
...
pub fn get_color_from_key_and_subkey(config: &NuConfig, key: &str, subkey: &str) -> Value {
    ...
    let mut v: Value = UntaggedValue::Primitive(nu_protocol::Primitive::String("nocolor".to_string())).into_value(Tag::unknown());
...
impl ConfigExtensions for NuConfig {
    ...
    fn header_style(&self) -> TextStyle {
        ...
        let head_color_style = lookup_ansi_color_style(head_color.as_string().unwrap_or("green".to_string()));
        ...
        TextStyle::new()
            ...
            .fg(head_color_style.foreground.unwrap_or(ansi_term::Color::Green))
    }
