...
pub fn convert_toml_value_to_nu_value(v: &toml_edit::Value, tag: impl Into<Tag>) -> Value {
    ...
    match v {
        toml_edit::Value::Boolean(b) => UntaggedValue::boolean(*b).into_value(tag),
        toml_edit::Value::Integer(n) => UntaggedValue::int(*n).into_value(tag),
        toml_edit::Value::Float(n) => UntaggedValue::decimal(*n).into_value(tag),
