use crate::completions::{
    to_reedline_span, CompletionOptions, Context, Fetched, MatchAlgorithm, NuMatcher, SemanticSuggestion,
};
...
impl CompleterOutput {
    ...
    fn read_options(&mut self, options: ReturnedOptions) {
        ...
        if let Some(positional) = options.positional {
            if !positional && self.options.match_algorithm == MatchAlgorithm::Prefix {
    ...
    pub(crate) fn into_fetched(self, ctx: &Context) -> Fetched {
        ...
        for suggestion in self.suggestions {
            ...
            if matcher.check_match(&value).is_some() {
                ...
                if let Some(description) = suggestion.suggestion.description.as_deref() {
                    if matcher.check_match(description).is_some() {
...
pub(crate) fn map_value_completions(
    ...
    list.filter_map(move |value| {
        ...
        value.coerce_into_string().ok().map(|string| SemanticSuggestion {
            suggestion: Suggestion {
