...
fn custom_completer() -> NuCompleter {
    ...
    // Add record value as example
    let record = "
        let external_completer = {|input, --full|
            ($input.contexts.tokens.text)
        }
...
fn custom_completions_wraps_builtin_commandline_complete_path() {
    ...
    let command = "
        def completer [input: record, --full] {
            ($input.contexts.tokens.text)
              | last
...
fn external_completer_trailing_space() {
    ...
    let block = "{|input, --full| ($input.contexts.tokens.text)}";
    let input = "gh alias ";
...
fn external_completer_wraps_builtin_commandline_complete() {
    let block = "{|input|
        $input.token.text | commandline complete | prepend 'bar'
    }";
    let input = "foo test";
...
fn command_wide_completion_custom(#[case] return_code: &str) {
    ...
    let sample = /* lang=nu */ format!(r#"
        def "nu-complete foo" [input: record, --full] {{
            {return_code} (($input.contexts.tokens.text) ++ [some more])
        }}
...
fn external_completer_receives_the_unified_input() {
    ...
    let block = r#"{|input, --full| [
        # `before |` is beside the cursor, not around it, so nothing nests.
        ($input.contexts.tokens.text | str join "+")
        ($input.contexts.tokens.kind | str join "+")
        $"nested=($input.contexts.tokens.nested | all { $in == null })"
        $"cursor=($input.place.cursor | to nuon)"
        $"place-kind=($input.place.kind)"
        $"target=($input.place.target | to nuon)"
    ]}"#;
...
fn a_completer_receives_the_contexts_enclosing_the_cursor() {
    ...
    let block = r#"{|input, --full| [
        ...
        ($input.contexts.tokens.kind | str join "+")
        $"slot=($input.contexts.tokens.1.nested | reject tokens | to nuon)"
...
fn the_text_after_the_cursor_never_reaches_the_completer() {
    let block = r#"{|input, --full| [($input.contexts.tokens.text | str join "+")]}"#;
    let cursor = "ext ".len();
...
fn alias_expansion_for_external_completions() {
    ...
    let command = "$env.config.completions.external.completer = {|input, --full| ($input.contexts.tokens.text) }";
    assert!(support::merge_input(command.as_bytes(), &mut engine, &mut stack).is_ok());
