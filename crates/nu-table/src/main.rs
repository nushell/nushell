...
fn main() {
    ...
    // The table headers
    let headers = vec_of_str_to_vec_of_styled_str(&table_headers, true);
    // The table rows
    let rows = vec_of_str_to_vec_of_styled_str(&row_data, false);
    // The table itself
...
fn vec_of_str_to_vec_of_styled_str(data: &[&str], is_header: bool) -> Vec<StyledString> {
    let mut v = vec![];
