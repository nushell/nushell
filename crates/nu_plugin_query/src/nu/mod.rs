...
impl Plugin for Query {
    fn signature(&self) -> Vec<PluginSignature> {
        vec![
            ...
            PluginSignature::build("query json")
                .usage(
                    "execute json query on json file (open --raw <file> | query json 'query \
                     string')",
                )
            ...
            PluginSignature::build("query web")
                ...
                .switch(
                    "inspect",
                    "run in inspect mode to provide more information for determining column \
                     headers",
                    Some('i'),
                )
    ...
    fn run(
        ...
        match name {
            "query" => self.query(name, call, input, path),
...
pub fn web_examples() -> Vec<PluginExample> {
    ...
    | query web --as-table [Rank City 'Population(2011)[3]' 'Population(2001)[3][a]' 'State or \
                      union territory']"
                .into(),
        ...
        PluginExample {
            example: "http get https://www.nushell.sh | query web --query 'h2, h2 + p' | group 2 \
                      | each {rotate --ccw tagline description} | flatten"
                .into(),
