...
impl PluginCommand for ExprMathBitwiseOr {
    ...
    fn examples(&self) -> Vec<Example<'_>> {
        vec![Example {
            ...
            example: "[[n]; [-1] [-0] [-1]] | polars into-df | polars select (polars col n | polars math bitwise-or | polars as n) | polars collect",
            result: Some(
                NuDataFrame::from(
                    df!("n" => [-1i64])
...
fn command_expr(
    ...
    NuExpression::from(expr.into_polars().bitwise_or())
