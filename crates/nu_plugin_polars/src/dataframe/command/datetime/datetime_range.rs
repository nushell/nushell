...
impl PluginCommand for DatetimeRange {
    type Plugin = PolarsPlugin;

    fn name(&self) -> &str {
        "polars datetime-range"
    }
    ...
    fn examples(&self) -> Vec<Example<'_>> {
        ...
        vec![
            Example {
                description: "Eagerly create a column of datetimes from a start, end, and interval",
                example: "polars datetime-range --start 2022-01-01 --end 2022-03-01 --interval 1mo --eager",
                result: Some(
                    NuDataFrame::new(
                        false,
                        df!("literal" => Series::new(
                            "".into(),
                            vec![dt(2022, 1, 1), dt(2022, 2, 1), dt(2022, 3, 1)],
                        )
                        .cast(&DataType::Datetime(TimeUnit::Nanoseconds, None))
                        .expect("datetime range example series should cast"))
                        .expect("datetime range example dataframe should build"),
                    )
                    .into_value(Span::test_data()),
                ),
            },
            Example {
                description: "Create a datetime range per group from the min and max of a date column",
                ...
                result: Some(
                    NuDataFrame::new(
                        ...
                        df!(
                            "key" => &["one", "two"],
                            "date_range" => &[
                                Series::new("".into(), vec![dt(2024, 1, 1), dt(2024, 1, 2)])
                                    .cast(&DataType::Datetime(TimeUnit::Nanoseconds, Some(timezone_utc())))
                                    .expect("datetime range example series should cast"),
                                Series::new(
                                    "".into(),
...
mod test {
    ...
    fn test_examples() -> Result<(), ShellError> {
        test_polars_plugin_command(&DatetimeRange)
    }
}
