...
use nu_protocol::{
    Config, FromValue, IntoValue, PipelineData, PipelineExecutionData, Span, Value,
    ast::Block,
...
pub fn test() -> NuTester {
    ...
    let tester = NuTester::with_engine_state(engine_state);
...
impl NuTester {
    ...
    pub(super) fn auto_load_plugins(self) -> Self {
        let mut tester = self;
...
impl NuTester {
    pub(super) fn with_engine_state(engine_state: EngineState) -> Self {
        Self {
            engine_state,
