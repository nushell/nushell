        ...
        let is_terminal = match (stdin, stdout, stderr) {
            ...
            _ => {
                let span = match call.arguments.len() {
                    0 => call.span(),
                    1 => *argument_span(call.arguments.first().unwrap()),
                    _ => {
                        // Build a span covering all arguments.  At least one of them must be removed
                        let first = argument_span(call.arguments.first().unwrap());
                        let last = argument_span(call.arguments.last().unwrap());
...
fn argument_span<'a>(argument: &'a Argument) -> &'a Span {
    match argument {
