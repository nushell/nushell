...
pub fn help_aliases(
    ...
    if let Some(f) = find {
        let all_cmds = build_help_aliases(engine_state, stack, head);
    ...
    if rest.is_empty() {
        let found_cmds = build_help_aliases(engine_state, stack, head);
...
fn build_help_aliases(engine_state: &EngineState, stack: &Stack, span: Span) -> List {
