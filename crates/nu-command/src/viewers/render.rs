...
impl Command for Render {
    ...
    fn extra_description(&self) -> &str {
        "If the input is a table and contains a column called 'index', this column is used as the table index instead of the usual continuous index."
    }
...
fn handle_render_command(mut input: CmdInput<'_>) -> ShellResult<PipelineData> {
    ...
    match input.data {
        ...
        PipelineData::Value(Value::Custom { val, .. }, ..) => {
            ...
            Render.run(input.engine_state, input.stack, input.call, base_pipeline)
        }
