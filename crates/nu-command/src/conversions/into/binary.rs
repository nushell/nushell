    ...
    fn signature(&self) -> Signature {
        Signature::build("into binary")
            ...
            .named(
                "endian",
                SyntaxShape::String,
                "byte encode endian. Does not affect string, date or binary. In containers, only \
                 individual elements are affected. Available options: native(default), little, big",
                Some('e'),
            )
            .rest(
                "rest",
