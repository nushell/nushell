    ...
    fn examples(&self) -> Vec<Example<'_>> {
        vec![
            ...
            Example {
                ...
                example: r#"# `fg`, `bg`, `attr` are the acceptable keys, all other keys are considered invalid and will throw errors. `fg` and `bg` take hexcode strings for color, and attr takes a list of attributes or a single string comprising of the attributes' shortcodes
            ...
            match k.as_str() {
                ...
                "attr" => {
                    if let Ok(attrs) = v.as_str() {
                        for attr in attrs.chars().map(String::from) {
                            nu_style.attr.push(attr);
                        }
                    } else if let Ok(raw_attrs) = v.as_list() {
                        for attr in raw_attrs.iter().map(Value::as_str) {
                            nu_style.attr.push(attr?.to_string());
                        }
                    } else {
