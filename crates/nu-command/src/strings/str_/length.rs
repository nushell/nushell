...
impl Command for SubCommand {
    ...
    fn signature(&self) -> Signature {
        Signature::build("str length")
            ...
            .switch(
                "grapheme-clusters",
                "count length using grapheme clusters (overrides 'grapheme_clusters' config option)",
                Some('g'),
            )
            .switch(
                "utf-8-bytes",
                "count length using UTF-8 bytes (overrides 'grapheme_clusters' config option)",
                Some('b'),
            )
            .rest(
                "rest",
