...
lazy_static! {
    static ref CHAR_MAP: IndexMap<&'static str, String> = indexmap! {
        ...
        "identical-to" => '\u{2261}'.to_string(),                  // ≡
        "hamburger" => '\u{2261}'.to_string(),                     // ≡
        "not-identical-to" => '\u{2262}'.to_string(),              // ≢
        "branch-untracked" => '\u{2262}'.to_string(),              // ≢
        "strictly-equivalent-to" => '\u{2263}'.to_string(),        // ≣
        "branch-identical" => '\u{2263}'.to_string(),              // ≣

        "upwards-arrow" => '\u{2191}'.to_string(),                 // ↑
        "branch-ahead" => '\u{2191}'.to_string(),                  // ↑
        "downwards-arrow" => '\u{2193}'.to_string(),               // ↓
        "branch-behind" => '\u{2193}'.to_string(),                 // ↓
        "up-down-arrow" => '\u{2195}'.to_string(),                 // ↕
        "branch-ahead-behind" => '\u{2195}'.to_string(),           // ↕

        "black-right-pointing-triangle" => '\u{25b6}'.to_string(), // ▶
        "prompt" => '\u{25b6}'.to_string(),                        // ▶
        "vector-or-cross-product" => '\u{2a2f}'.to_string(),       // ⨯
        "failed" => '\u{2a2f}'.to_string(),                        // ⨯
        "high-voltage-sign" => '\u{26a1}'.to_string(),             // ⚡
