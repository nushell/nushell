...
pub fn grapheme_flags(call: &Call) -> Result<bool, ShellError> {
    ...
    // Note that Nushell already prevents nonexistent flags from being used with
    // commands, so this function can be reused for both the --utf-8-bytes
    // commands and the --code-points commands.
