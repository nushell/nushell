...
fn parse(
    ...
    let url = match base_url {
        Some(base_url) => {
            let base = Url::parse(base_url.as_str()).map_err(|_| {
                ShellError::UnsupportedInput {
                    msg: "Incomplete or incorrect base URL. Expected a full URL, e.g., https://www.example.com".to_string(),
            ...
            base.join(url_string.as_str()).map_err(|_| ShellError::UnsupportedInput {
                msg: "Incomplete or incorrect URL. Expected a valid URL, or a relative URL when using --base"
                    .to_string(),
                input: "value originates from here".into(),
                msg_span: head,
                input_span: span,
        ...
        None => Url::parse(url_string.as_str()).map_err(|_| ShellError::UnsupportedInput {
