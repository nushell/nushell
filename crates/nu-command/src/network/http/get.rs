...
use crate::network::http::client::{
    http_client_tmp, http_parse_redirect_mode, http_parse_url, request_add_authorization_header,
    ...
    fn signature(&self) -> Signature {
        Signature::build("http get")
            ...
            .named(
                "redirect-mode",
                SyntaxShape::String,
                "What to do when encountering redirects. Redirects are followed by default. Valid options: 'follow' ('f'), 'manual' ('m'), 'error' ('e').",
                None
            )
            .filter()
...
fn helper(
    ...
    let client = http_client_tmp(args.insecure, redirect_mode, engine_state, stack);
