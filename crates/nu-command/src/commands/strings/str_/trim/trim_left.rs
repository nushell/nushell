...
fn trim_left(s: &str, char_: Option<char>, _all_flag: bool) -> String {
    match char_ {
...
mod tests {
    ...
    fn trims_whitespace_from_left() {
        ...
        let actual = action(&word, Tag::unknown(), None, false, &trim_left, ActionMode::Local).unwrap();
    ...
    fn trims_left_global() {
        ...
        let actual = action(&word, Tag::unknown(), None, false,  &trim_left, ActionMode::Global).unwrap();
        assert_eq!(actual, expected);
    }
    ...
    fn global_trim_left_row() {
        ...
        let actual = action(&row, Tag::unknown(), None, false, &trim_left, ActionMode::Global).unwrap();
        assert_eq!(actual, expected);
    }
