...
impl WholeStreamCommand for SubCommand {
    ...
    fn signature(&self) -> Signature {
        Signature::build("str to-bytes")
            .named("radix", SyntaxShape::Number, "radix of integer", Some('r'))
            .rest(
                "rest",
                SyntaxShape::ColumnPath,
                "optionally convert text into bytes by column paths",
            )
    }
    ...
    fn examples(&self) -> Vec<Example> {
        vec![
            Example {
                ...
                example: "echo '255' | str to-bytes",
                result: Some(vec![UntaggedValue::filesize(255).into()]),
            },
            Example {
                description: "Convert str column to an integer",
                example: "echo [['count']; ['255']] | str to-bytes count | get count",
                result: Some(vec![UntaggedValue::filesize(255).into()]),
            },
            Example {
                description: "Convert to integer from binary",
                example: "echo '1101' | str to-bytes -r 2",
                result: Some(vec![UntaggedValue::filesize(13).into()]),
            },
            Example {
                description: "Convert to integer from hex",
                example: "echo 'FF' | str to-bytes -r 16",
                result: Some(vec![UntaggedValue::filesize(255).into()]),
            },
        ]
    }
        ...
        UntaggedValue::Primitive(Primitive::String(s)) => {
            ...
            let out = match trimmed {
                b if b.starts_with("0b") => {
                    let num = match u64::from_str_radix(b.trim_start_matches("0b"), 2) {
                        Ok(n) => n,
                        Err(reason) => {
                    ...
                    UntaggedValue::filesize(num)
                }
                h if h.starts_with("0x") => {
                    let num = match u64::from_str_radix(h.trim_start_matches("0x"), 16) {
                        Ok(n) => n,
                        Err(reason) => {
                            return Err(ShellError::labeled_error(
                ...
                _ => {
                    let num = match u64::from_str_radix(trimmed, radix) {
                        Ok(n) => n,
                        Err(reason) => {
                            return Err(ShellError::labeled_error(
                                "could not parse as int",
...
mod tests {
    ...
    use nu_source::Tag;
    use nu_test_support::value::{bytes, string};
    ...
    fn turns_hex_to_integer() {
        ...
        assert_eq!(actual, bytes(255));
    }

    #[test]
    fn communicates_parsing_error_given_an_invalid_integerlike_string() {
        let byte_str = string("36anra");

        let actual = action(&byte_str, Tag::unknown(), 10);
