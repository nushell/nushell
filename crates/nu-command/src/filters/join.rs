...
use nu_protocol::{
    Config, Example, PipelineData, Record, ShellError, Signature, Span, SyntaxShape, Type, Value,
};
use std::cmp::max;
...
fn join_rows(
    ...
    for this_row in this {
        if let Value::Record { val: this_rec, .. } = this_row {
            if let Some(this_valkey) = this_row.get_data_by_key(this_join_key) {
                if let Some(other_rows) = other.get(&this_valkey.into_string(sep, config)) {
                        ...
                        for other_rec in other_rows {
                            ...
                            let record = match join_type {
                                JoinType::Inner | JoinType::Right => merge_records(
                                    other_rec, // `other` (lookup) is the left input table
                                    this_rec,
                                    shared_join_key,
                                ),
                                JoinType::Left => merge_records(
                                    this_rec, // `this` is the left input table
                                    other_rec,
                                    shared_join_key,
                                ),
                    ...
                    let other_rec = Record {
                        cols: other_keys.to_vec(),
                        vals: other_vals,
                    };

                    let record = match join_type {
                        JoinType::Inner | JoinType::Right => {
                            merge_records(&other_rec, this_rec, shared_join_key)
                        }
                        JoinType::Left => merge_records(this_rec, &other_rec, shared_join_key),
                        _ => panic!("not implemented"),
                    };
...
fn column_names(table: &[Value]) -> &[String] {
    table
        ...
        .find_map(|val| match val {
            Value::Record { val, .. } => Some(val.cols.as_slice()),
            _ => None,
        })
        .unwrap_or(&[])
}
