        ...
        match input {
            PipelineData::Value(Value::List { .. }, ..) | PipelineData::ListStream { .. } => {
                let res = input.into_iter().map(move |x| match x.as_record() {
                    Ok(record) => {
                        move_record_columns(record, &columns, &before_or_after, call.head)
...
fn move_record_columns(
    ...
    let mut output = Record::with_capacity(record.len());

    for (i, (col, val)) in record.iter().enumerate() {
        match &before_or_after.item {
            BeforeOrAfter::After(after) if after == col => {
                output.push(col.clone(), val.clone());
            ...
            BeforeOrAfter::Before(before) if before == col => {
                for idx in column_idx.iter() {
                    if let (Some(col), Some(val)) = (record.cols.get(*idx), record.vals.get(*idx)) {
                        output.push(col.clone(), val.clone());
                    } else {
