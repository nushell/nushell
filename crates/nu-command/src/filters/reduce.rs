...
impl Command for Reduce {
    ...
    fn examples(&self) -> Vec<Example> {
        vec![
            ...
            Example {
                ...
                result: Some(SpannedValue::test_string("ArXhur, KXng Xf Xhe BrXXXns")),
            },
            Example {
                example: r#"['foo.gz', 'bar.gz', 'baz.gz'] | enumerate | reduce -f '' {|str all| $"($all)(if $str.index != 0 {'; '})($str.index + 1)-($str.item)" }"#,
                description:
                    "Add ascending numbers to each of the filenames, and join with semicolons.",
                result: Some(SpannedValue::test_string("1-foo.gz; 2-bar.gz; 3-baz.gz")),
            },
            Example {
                ...
                result: Some(SpannedValue::test_string("StrStrStr")),
            },
        ]
    }
    ...
    fn run(
        ...
        let fold: Option<SpannedValue> = call.get_flag(engine_state, stack, "fold")?;
