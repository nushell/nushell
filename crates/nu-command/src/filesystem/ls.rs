...
impl Command for Ls {
    ...
    fn signature(&self) -> nu_protocol::Signature {
        Signature::build("ls")
            ...
            .switch(
                "numeric",
                "Use integer values for uid and gid, octal output for mode (only affects `-l`)",
                Some('n'),
            )
            .switch(
                "short-names",
    ...
    fn examples(&self) -> Vec<Example<'_>> {
        vec![
            ...
            Example {
                description: "Add the binary mode of the file",
                example: "ls -ln | upsert binary_mode {|s| $s.mode | into int | format number | get binary}",
                result: None,
            },
        ]
    }
    ...
    if long && let Some(md) = metadata {
        ...
        #[cfg(unix)]
        {
            ...
            record.push(
                "mode",
                if numeric {
                    Value::string(format!("{mode:#o}"), span)
                } else {
