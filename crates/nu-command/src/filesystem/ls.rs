...
fn ls_for_one_pattern(
    ...
    let (pattern_arg, absolute_path) = match pattern_arg {
        Some(pat) => {
            ...
            if !directory && tmp_expanded.is_dir() {
                if read_dir(tmp_expanded, p_tag, use_threads, signals)?
                    .next()
                    .is_none()
                {
        ...
        None => {
            ...
            } else if read_dir(cwd.clone(), p_tag, use_threads, signals)?
                .next()
                .is_none()
            {
                return Ok(Value::test_nothing().into_pipeline_data());
    ...
    let (prefix, paths) = if just_read_dir {
        ...
        let paths = read_dir(expanded.clone(), p_tag, use_threads, signals)?;
...
fn read_dir(
    ...
    if !use_threads {
        ...
        collected.sort_by(|a, b| match (a, b) {
            _ if signals.interrupted() => Ordering::Equal,
            (Ok(a), Ok(b)) => a.cmp(b),
