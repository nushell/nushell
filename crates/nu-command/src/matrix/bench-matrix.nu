...
# 1. SUM
let r1 = (bench { matrix zeros 100 100 | matrix add 1 | matrix sum } { 1..100 | each {|_el| 1..100 | each {|_el| 1.0}} | flatten | math sum } --rounds 100)
...
# 2. SCALE
let r2 = (bench { matrix zeros 100 100 | matrix add 1 | matrix scale 2 | matrix into-nu | collect } { 1..100 | each {|_el| 1..100 | each {|_el| 1.0}} | each {|row| $row | each {|e| $e * 2}} | collect } --rounds 100)
...
# 6. CONSTRUCT
let r6 = (bench { matrix zeros 100 100 | matrix add 1 | matrix into-nu | collect } { 1..100 | each {|_el| 1..100 | each {|_el| 1.0}} | collect } --rounds 100)
...
# 7. AXIS SUM
let r7 = (bench { matrix zeros 100 100 | matrix add 1 | matrix sum --axis 1 | matrix into-nu | collect } { 1..100 | each {|_el| 1..100 | each {|_el| 1.0}} | each {|row| $row | math sum} | collect } --rounds 100)
...
# 8. MAP
let r8 = (bench { matrix zeros 100 100 | matrix add 1 | matrix map {|e| $e * 2 + 1} | matrix into-nu | collect } { 1..100 | each {|_el| 1..100 | each {|_el| 1.0}} | each {|row| $row | each {|e| $e * 2 + 1}} | collect } --rounds 100)
