...
fn out_html_metadata() -> Result {
    ...
    test().run(code).expect_value_eq("text/html; charset=utf-8")
}
...
fn out_html_partial() -> Result {
    ...
    test()
        .run(code)
        .expect_value_eq("<div style=\"background-color:white;color:black;\">3</div>")
}
...
fn out_html_table() -> Result {
    ...
    test().run(code).expect_value_eq(
        "<html><style>body { background-color:white;color:black; }</style><body><table><thead><tr><th>name</th></tr></thead><tbody><tr><td>darren</td></tr></tbody></table></body></html>",
    )
}
