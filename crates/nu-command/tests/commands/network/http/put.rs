...
fn http_put_follows_redirect() {
    ...
    let actual = nu!("http put {}/foo putbody", server.url());
...
fn http_put_redirect_mode_manual() {
    ...
    let actual = nu!(
        "http put --redirect-mode manual {}/foo putbody",
        server.url()
    );
...
fn http_put_redirect_mode_error() {
    ...
    let actual = nu!(
        "http put --redirect-mode error {}/foo putbody",
        server.url()
    );
...
fn http_put_timeout() {
    ...
    let actual = nu!("http put --max-time 100ms {} putbody", server.url());
