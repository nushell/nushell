...
fn keeps_nulls_in_lists() -> Result {
    test()
        .run(r#"[null, 2, 3] | default []"#)
        .expect_value_eq(((), 2, 3))
}

#[test]
fn replaces_null() -> Result {
    test().run(r#"null | default 1"#).expect_value_eq(1)
}
...
fn replace_empty_string() -> Result {
    test().run(r#"'' | default -e foo"#).expect_value_eq("foo")
}

#[test]
fn do_not_replace_empty_string() -> Result {
    test().run(r#"'' | default 1"#).expect_value_eq("")
}
...
fn do_not_replace_empty_list() -> Result {
    test().run(r#"[] | default 1 | length"#).expect_value_eq(0)
}
...
fn do_not_replace_empty_record() -> Result {
    test()
        .run(r#"{} | default {a:5} | columns | length"#)
        .expect_value_eq(0)
}
...
fn closure_eval_complex() -> Result {
    test()
        .run(r#"null | default { seq 1 5 | math sum }"#)
        .expect_value_eq(15)
}
...
fn closure_eval_is_lazy() -> Result {
    test()
        .run(r#"1 | default { error make -u {msg: foo} }"#)
        .expect_value_eq(1)
}
...
fn column_closure_eval_is_lazy() -> Result {
    test()
        .run(r#"{a: 1} | default { error make -u {msg: foo} } a | get a"#)
        .expect_value_eq(1)
}
...
fn closure_eval_do_not_replace_empty_record() -> Result {
    test()
        .run(r#"{} | default { 1 } | columns | length"#)
        .expect_value_eq(0)
}
